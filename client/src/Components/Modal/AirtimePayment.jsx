...
function AirtimePayment({formData, setFormData}) {
  ...
  return (
    <form onSubmit={handleAirtime} className="absolute left-0 top-0 flex w-full h-[100%] flex-col items-center">
      ...
      <h1 className="flex items-center gap-1 text-[36px] font-semibold text-main-color" ><FaNairaSign />{ formData.useCashback ? Math.round(formData?.airtimeValue - cashPoint).toFixed(2) : Math.round(formData?.airtimeValue).toFixed(2) }</h1>
      {
        formData.useCashback && (
          <h3 className="font-semibold text-gray-400 line-through text-[24px] flex items-center gap-1 mb-2 phone:text-[20px]"><FaNairaSign /> {Math.round(formData?.airtimeValue).toFixed(2)}</h3>
        )
      }
      ...
      <div className="w-full overflow-y-auto">
          <div className="w-full bg-white p-4 rounded-2xl flex flex-col gap-3">
            <div className="flex items-center justify-between">
              ...
              <p className="flex items-center text-[20px] phone:text-[17px] font-medium"><FaNairaSign className="text-[16px]" />{formData?.airtimeValue}</p>
            </div>
            <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col mt-8">
            ...
            <div className="flex items-center gap-2 relative">
              ...
              <p className="text-yellow-500 font-bold text-[17px]">+{Math.round((discount*formData.airtimeValue)/100)}Pts</p>
            </div>
          </div>
