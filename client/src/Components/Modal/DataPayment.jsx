...
function DataPayment({formData, setFormData}) {
  ...
  return (
    <form onSubmit={handleData} className="absolute left-0 top-0 flex w-full h-[100%] flex-col items-center">
      ...
      <h1 className="flex items-center gap-1 text-[36px] small-phone:text-[26px] font-semibold text-main-color" ><FaNairaSign />{ formData.useCashback ? Math.round(formData?.dataPrice - cashPoint).toFixed(2) : Math.round(formData?.dataPrice).toFixed(2) }</h1>
      {
        formData.useCashback && (
          <h3 className="font-semibold text-gray-400 line-through text-[24px] flex items-center gap-1 mb-2 phone:text-[20px] small-phone:text-[17px]"><FaNairaSign /> {Math.round(formData?.dataPrice).toFixed(2)}</h3>
        )
      }

      <div className="w-full overflow-y-auto">
          <div className="w-full bg-white p-4 rounded-2xl flex flex-col gap-3 phone:gap-4">
            <div className="flex items-center justify-between">
              <p className="text-[20px] font-light phone:text-[17px] small-phone:text-[14px]">Amount</p>
              <p className="flex items-center text-[20px] phone:text-[17px] font-medium small-phone:text-[14px]"><FaNairaSign className="text-[16px] small-phone:text-[13px]" />{Math.round(formData?.dataPrice).toFixed(2)}</p>
            </div>
            <div className="flex items-center justify-between">
              <p className="text-[20px] font-light phone:text-[17px] small-phone:text-[14px]">Provider</p>
              <p className="font-medium text-[22px] phone:text-[17px] small-phone:text-[14px] flex gap-1.5 items-center">
                <img className="w-[20px] small-phone:w-[15px]" alt={formData?.network} src={formData?.icon} />
                {formData?.network}
              </p>
            </div>
            <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between">
              <p className="text-[20px] font-light phone:text-[17px] small-phone:text-[14px]">Bundle Details</p>
              <p className="font-medium text-[20px] phone:text-[17px] small-phone:text-[13px]">{formData?.bundleDetail}</p>
            </div>
          </div>
          ...
          <div className="w-full mt-8 bg-white p-3 rounded-2xl flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <p className={`flex items-center text-[17px] font-medium small-phone:text-[13px] ${formData.useCashback ? 'line-through text-gray-400' : ''}`}><FaNairaSign className="small-phone:text-[12px]" /> {Math.round(cashPoint).toFixed(2)} availble</p>
          ...
          <p className="small-phone:text-[14px] font-semibold">Earn</p>
            ...
            <div className="flex items-center gap-2 small-phone:gap-1">
              ...
              <p className="text-yellow-500 font-bold text-[17px] small-phone:text-[14px]">+{formData?.discountAllowed}Pts</p>
            </div>
          </div>
